...
int unit(int len, char const *p)
{
	...
	tmp___3 = number(p, len);   // 0 o 1
	if (tmp___3) {
	    if (!lflag) {
            encoder(88, encodeStrings_litStr88);    // numero a parola
...
void convert(char *line)
{
	...
	while (1) {
	    ...
	    if ((int) ((unsigned char) *p) >= 48) {
            if ((int) ((unsigned char) *p) <= 57) { //input is a number
                goto __Cont;
            }
	    }
	    switch ((int) *p) {
	...
	*p = (char) '\000';
	...
	if (len > 65) {
	    ...
	    exit(-1);
	} else if ((unsigned long) fraction != (unsigned long) ((void *) 0)) {
	...
	if (!rval) {
		...
		encoder(13, encodeStrings_litStr13);    //punto finale
