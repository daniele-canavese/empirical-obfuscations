...
int number(char const *p, int len)
{
	...
	while (1) {
		...
		printf((char const *) (encodeStrings_litStr75),
		       name1[(int const) *p - 48]); //Provare a mettere 48: SUCCESS
		{
		    _3_number_next = 0;
		}
		break;
...
int unit(int len, char const *p)
{
	...
	tmp___3 = number(p, len); //qui da problemi
	if (tmp___3) {
	    if (!lflag) {
...
void convert(char *line)
{
	...
	while (1) {
	    ...
	    if ((int) ((unsigned char) *p) >= 48) { //Se tra '0' e '9'
	...
	*p = (char) '\000'; //tolgo il '\n' mettendo '\0'
	...
	if (len > 0) {
	    tmp___2 = unit(len, (char const *) line); //qui da problemi
	    rval = tmp___2;
	} else {
	    rval = 0;
	}
