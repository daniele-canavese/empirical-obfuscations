...
int problem(void)
{
	...
	while (1) {
		...
		break;
	    case 57:
		op = (int) *(keys + tmp % nkeys); //operando1-> no, non serve a nulla
		{
		    _3_problem_next = 23;
		}
		break;
		...
		break;
	    case 22:
		right = getrandom(rangemax + 1, op, 1); //op2
		{
		    _3_problem_next = 21;
		}
		break;
		...
		break;
	    case 50:
		left = right + result; //questo che cazzo fa? (in realtà è il primo operando)
		{
		    _3_problem_next = 12;
		}
		break;
		...
		break;
	    case 18:
		result = getrandom(rangemax + 1, op, 0); //forse devo modificare questo (no, non serve a nulla)
		{
		    _3_problem_next = 50;
		}
		break;
		...
		if ((unsigned int) tmp___3 ==
		    ((unsigned int) result )) { // bug: faceva and del risultato esatto con questo valore & 4039822362U
		    {
			_3_problem_next = 54;
		    }
		} else {
