...
int number(char const *p, int len)
{
	...
	while (1) {
		...
		break;
	    case 6:
		/*printf((char const *) (encodeStrings_litStr75),  // %s
		       name1[(int const) *p - 47]); // qui dovrebbe sottrarre 48 invece che 47
		       */
        printf((char const *) (encodeStrings_litStr75),  // %s
		       name1[(int const) *p - 48]);
		{
...
void convert(char *line)
{
	...
	while (1) {
	    ...
	    if ((int) ((unsigned char) *p) >= 48) { //'0'
		if ((int) ((unsigned char) *p) <= 57) { // '9'
		    goto __Cont;
		}
	    }
	    switch ((int) *p) {
	  ...
	  __Cont:
	    p++; //incremento il puntatore. Perdo la prima cifra
	}
	*p = (char) '\000';
