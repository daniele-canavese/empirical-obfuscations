...
int number(char const *p, int len)
{
	...
	{
	    _5_number_next =
		(unsigned long) (_4_number__opaque_array[((unsigned int) (((_3entropy - 8) + ((_3entropy - 8) >> 63L)) ^ ((_3entropy - 8) >> 63L)) % 10) * 3 +0] % _4_number__opaque_array[5] + 15);
	}
	while (1) {
		...
		rval = 0;
		{
		    _5_number_next =
			(unsigned
			 long) (_4_number__opaque_array[((unsigned int) ((len ^ (len >> 31)) - (len >> 31)) % 10) * 3 + 1] % _4_number__opaque_array[11] + 12);
		}
		break;
	    case 17:;
		switch (len) {
