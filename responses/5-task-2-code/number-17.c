...
int number(char const *p, int len)
{
	...
	while (1) {
		...
		break;
	    case 2:;
		if ((int const) *p != 48) { //se quindi numero che ho messo diverso da 0
		    {
			_3_number_next = 1;
		    }
		} else {
...
void convert(char *line)
{
	...
	while (1) {
	    if ((int) *p != 0) {
		if (!((int) *p != 10)) { //quando raggiunge il fine riga
		    break;
		}
	    } else {
		break;
		...
		if ((int) ((unsigned char) *p) <= 57) { //se quindi è un numero da 1 a 9
		    goto __Cont;
		}
	    }
	    switch ((int) *p) {
	...
	*p = (char) '\000';
	...
	if (len > 65) {
	    ...
	    exit(-1);
		   ...
		   (unsigned long) ((void *) 0)) { //se fraction != NULL
