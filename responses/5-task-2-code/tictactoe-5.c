...
char check(void)
{
	...
	{
	    _5_check_next =
		(unsigned long) (_4_check__opaque_array[((unsigned int) ((i ^ (i >> 31)) - (i >> 31)) % 10) * 3 + 1] %
	...
	while (1) {
		...
		break;
	    case 12:;
		return (1); // fatto nel caso di vittoria, ritorna 79 'O'
		break;
	    case 11:
		i++;
		{
		...
		break;
	    case 0:;
		return ((char) ' '); // fatto quando non c'è nulla per player e pc
		break;
	    }
	}
    }
